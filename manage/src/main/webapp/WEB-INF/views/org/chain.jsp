                   ...
                   { link: true, display: "创建同级节点", css: "icon-plus", showIcon: true, url: "javascript:;", 
                    click: function() {
                        ...
                        if(parentId != undefined) 
                            location.href = "<%= UrlHelper.resolveWithReturnUrl("/org/add/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), "expanded={1}", pageContext)%>".replace("{0}", parentId).replace(escape("{1}"), expanded);
                        else
                            alert("必须选择择一个节点！");
                        return false;
                      } 
                   ...
                   { link: true, display: "创建子级节点", css: "icon-plus", showIcon: true, url: "javascript:;", 
                    click: function() {
                        ...
                        if(selectedId != undefined)
                        	location.href = "<%= UrlHelper.resolveWithReturnUrl("/org/add/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), "expanded={1}", pageContext)%>".replace("{0}", selectedId).replace(escape("{1}"), expanded);
                        else
                            alert("必须选择择一个节点！");
                        return false;
                       }
                    ...
                    { link: true, display: "编辑", css: "icon-edit", showIcon: true, url: "javascript:;",
                     click: function() {
                        ...
                        var expanded = $('#treeData-list').treeLite('expandedIds');
                        if(selectedId != undefined) 
                        	location.href = "<%= UrlHelper.resolveWithReturnUrl("/org/edit/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), "expanded={1}", pageContext)%>".replace("{0}", selectedId).replace(escape("{1}"), expanded);
                        else
                            alert("必须选择择一个节点！");
                        return false;
                        } 
                    ...
                    { link: true, display: "删除", css: "icon-trash", showIcon: true, url: "javascript:;",
                     click: function() {
                        ...
                        {
                            if(confirm("确认删除所选节点？"))
                            	location.href = "<%= UrlHelper.resolveWithReturnUrl("/org/delete/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), "expanded={1}", pageContext)%>".replace("{0}", selectedId).replace(escape("{1}"), expanded);
                        }
                        else
                            alert("必须选择择一个节点！");
                        return false;
